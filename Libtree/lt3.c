...
tree_print_recurse(struct fileinfo finfo)
{
  ...
  /* TODO: print the path info */         
    if (print_path_info(finfo) == -1) goto exit;
    
  /* TODO: continue ONLY if path is a directory */
    if (!(S_ISDIR(finfo.st.st_mode))) {
      putchar('\n');
     // if (strcmp(finfo.path,".comply") == 0) printf("||||| %d\n", errno);
      goto exit;}
  // printf("%d", !(finfo.st.st_mode & S_IXOTH));   
//if (strcmp(finfo.path,".consensus") == 0) printf("||||| %d  %s\n", errno, finfo.path);
//  if (strcmp(finfo.path, "favorites") == 0) {printf("out  %d-------",finfo.st.st_mode);}
  if ((dir = openat(cur_dir, finfo.path, O_RDONLY | O_CLOEXEC)) == -1  ||
      (dirp = fdopendir(dir)) == NULL || (finfo.st.st_mode & S_IXOTH) == 0) {
    if (errno == EACCES || (finfo.st.st_mode & S_IXOTH) == 0 || (errno == ENOTDIR)) {
   // if (strcmp(finfo.path,".comply") == 0) printf("||||| %d  %s   %d\n", errno, finfo.path, dir);
      ...
      if(dir != -1 && dirp != NULL){
        ...
        while((d = readdir(dirp)) != NULL){
         if (strcoll(d->d_name, ".") != 0 && strcoll(d->d_name, "..") != 0 && (opts.all|| strncmp(d->d_name, ".", 1) != 0))
         { n++;}
          /* if (strcmp(finfo.path, "adapter") == 0) {printf("  %d-----%d-----%s",finfo.st.st_mode, n, d->d_name  );} 
           if (strcmp(finfo.path, "statutes") == 0) {printf("  %d-----%d-----%s",finfo.st.st_mode, n, d->d_name  );}
           if (strcmp(finfo.path, "favorites") == 0) {printf("  %d-----%d-----%s",finfo.st.st_mode, n, d->d_name  );} 
           if (strcmp(finfo.path, "saturn") == 0) {printf("  %d-----%d-----%s",finfo.st.st_mode, n, d->d_name   );}  
           
           if (strcmp(finfo.path, "swiss") == 0) {printf("  %d-----%d-----%s",finfo.st.st_mode, n, d->d_name );} 
           if (strcmp(finfo.path, "plans") == 0) {printf("  %d-----%d---%s",finfo.st.st_mode, n, d->d_name);} */
...
out:
 ...
 // if (strcmp(finfo.path,".consensus") == 0) printf("||||| %d   %s %d\n", errno, finfo.path, dir);
/*if (strcmp(finfo.path,".consensus") == 0){
     printf(" [could not open directory %s]\n", finfo.path);
     goto exit;}*/
...
print_path_info(struct fileinfo finfo)
{
  ...
  if (opts.user) {
    /*  Hint: getpwuid(3) */
    if (printf("%c%s", sep, getpwuid(finfo.st.st_uid)->pw_name) < 0) goto exit; /* TODO */
    sep = ' ';
  }
  if (opts.group) {
    /*  Hint: getgrgid(3) */
    if (printf("%c%s", sep, getgrgid(finfo.st.st_gid)->gr_name) < 0) goto exit; /* TODO */
    sep = ' ';
  }           
  if (opts.size) {
    /*  Hint: stat.h(0p) */
    if (printf("%c%jd", sep, (finfo.st.st_size)-0) < 0) goto exit; /* TODO */
    sep = ' ';
  }
  if (sep != '[')
...
read_file_list(DIR *dirp, struct fileinfo **file_list, size_t *file_count)
{
  for (;;) {
    ...
    //if (strcmp(de,".consensus") == 0) printf("||||| %d   %s\n", errno, de);
    ...
    if (fstatat(cur_dir, de->d_name, &(*file_list)[(*file_count) - 1].st, AT_SYMLINK_NOFOLLOW) == -1) break;
        //if (strcmp(de->d_name,".consensus") == 0) printf("|||||\n");
